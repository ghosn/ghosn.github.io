function sample = zen_get_samples(mtx)
%
...
b_driving(isnan(Lat)) = 0;
b_driving(isnan(Long)) = 0;
...
if isempty(Lat_v) || isempty(Long_v),
    return;
end
...
dE = -(Eres_v(2:end)-Eres_v(1:end-1))./1000;
%Eff = dE./dist;
%Eff = dist./dE;

%figure; plot(dist, Eff);

sample = [start_pt, end_pt, dE, dist];

return;
%saveto = [csv_file(1:end-4), '.json'];
%zen_write_json(sample, saveto);
